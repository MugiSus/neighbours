[...document.getElementsByClassName("slider-container")].forEach(
  (container) => {
    container.getElementsByClassName("slider-value")[0].textContent =
      container.dataset.value;
    container.getElementsByClassName("slider-bar")[0].style.width = `${
      ((container.dataset.value - container.dataset.min) /
        (container.dataset.max - container.dataset.min)) *
      100
    }%`;
    ...
    let eventFunction = (event) => {
      if (event.buttons == 1) {
        ...
        let value =
          Math.round(
            (ratio * (container.dataset.max - container.dataset.min) +
              container.dataset.min * 1) /
              container.dataset.step
          ) * container.dataset.step;
        container.dataset.value = container.dataset.toFixed
          ? value.toFixed(container.dataset.toFixed)
          : value;
        container.getElementsByClassName("slider-value")[0].textContent =
          container.dataset.value;
        container.getElementsByClassName("slider-bar")[0].style.width = `${
...
document
  .getElementById("show-division-infos-container")
  .addEventListener("mouseup", (event) => {
    areInfomationsEnabled ^= true;
    document.getElementById("show-division-infos-text").textContent =
      areInfomationsEnabled ? "Hide division infos" : "Show division infos";
  });
...
function updateSimulationInfos() {
  ...
  document.getElementById("neighbour-tested-value").textContent =
    neighbourTestCount;
}
